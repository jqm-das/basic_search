...
def create_grid():
	...
	for i in range(0, 800, 8):
		for j in range(0, 800, 8):
			line = canvas.create_line([(i, j), (i+8, j)], tag='grid_line')
			lines.append(line)
	...
	for i in range(0, 800, 8):
		for j in range(0, 800, 8):
			line = canvas.create_line([(j, i), (j, i+8)], tag='grid_line')
			lines.append(line)
...
canvas = tk.Canvas(root, height=2500, width=2500, bg='white') #creates a canvas widget, which can be used for drawing lines and shapes
create_grid()
